...
- Story includes all database-specific implementation requirements
...
- Database best practices are followed
...
- [ ] Database requirements are explicitly stated

### Implementation Clarity (25 points)
- [ ] Story provides enough database detail for implementation
- [ ] Technical constraints and assumptions are explicit
- [ ] Database patterns and approaches are specified
...
- `implementation-clarity` - Missing database details, unclear requirements
...
- `best-practices` - Violates database standards
...
```json
{
  ...
  "strengths": ["What the Story does well from database perspective"],
  "improvementPriorities": ["Top 3 improvements ranked by impact"],
  "readyForImplementation": boolean,
  "estimatedComplexity": "low|medium|high",
  "domainSpecificNotes": "Any additional database context or implementation guidance"
}
```

## Scoring Guidelines

- **90-100 (Excellent)**: Crystal clear acceptance criteria, all database details specified, highly testable
- **70-89 (Acceptable)**: Core requirements clear, minor gaps acceptable, implementable with clarification
- **0-69 (Needs Improvement)**: Critical ambiguities, missing database requirements, must fix before implementation
...
```json
{
  ...
  "issues": [
    ...
    },
    {
      ...
      "description": "Missing database implementation details: user lookup query, credential verification",
      "suggestion": "Add technical requirements specific to database.",
      "example": "Technical: Query users table by email, verify password_hash"
    },
    {
      "severity": "major",
  ...
  "improvementPriorities": [
    ...
    "2. Add database-specific implementation details",
